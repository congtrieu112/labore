...
class ControllerQuickCheckoutCheckout extends Controller { 
	public function index() {
		...
		if (!$this->config->get('quickcheckout_debug')) {
		...
		$this->data['text_already_have_account'] = $this->language->get('text_already_have_account');
		...
		if ($this->config->get('quickcheckout_countdown')) {
			$this->data['timezone'] = substr(date('O'), 0, 3);
			...
			if (!empty($text[$this->config->get('config_language_id')])) {
				if ($this->config->get('quickcheckout_countdown_start')) {
					...
					if (time() > strtotime($time)) {
						$this->data['countdown_end'] = date('d M Y H:i:s', (strtotime($time) + 86400));
					} else {
		...
		// All variables
		$this->data['load_screen'] = $this->config->get('quickcheckout_load_screen');
		$this->data['edit_cart'] = $this->config->get('quickcheckout_edit_cart');
		$this->data['highlight_error'] = $this->config->get('quickcheckout_highlight_error');
		$this->data['text_error'] = $this->config->get('quickcheckout_text_error');
		$this->data['layout'] = $this->config->get('quickcheckout_layout');
		$this->data['slide_effect'] = $this->config->get('quickcheckout_slide_effect');
		$this->data['debug'] = $this->config->get('quickcheckout_debug');
		$this->data['auto_submit'] = $this->config->get('quickcheckout_auto_submit');
		$this->data['coupon_module'] = $this->config->get('quickcheckout_coupon');
		$this->data['voucher_module'] = $this->config->get('quickcheckout_voucher');
		$this->data['reward_module'] = $this->config->get('quickcheckout_reward');
		$this->data['cart_module'] = $this->config->get('quickcheckout_cart');
		$this->data['payment_module'] = $this->config->get('quickcheckout_payment_module');
		$this->data['shipping_module'] = $this->config->get('quickcheckout_shipping_module');
		$this->data['login_module'] = $this->config->get('quickcheckout_login_module');
		$this->data['delivery'] = $this->config->get('quickcheckout_delivery');
		$this->data['delivery_time'] = $this->config->get('quickcheckout_delivery_time');
		$this->data['delivery_min'] = $this->config->get('quickcheckout_delivery_min');
		$this->data['delivery_max'] = $this->config->get('quickcheckout_delivery_max');
		$this->data['delivery_min_hour'] = $this->config->get('quickcheckout_delivery_min_hour');
		$this->data['delivery_max_hour'] = $this->config->get('quickcheckout_delivery_max_hour');
		$this->data['delivery_unavailable'] = $this->config->get('quickcheckout_delivery_unavailable');
		$this->data['countdown'] = $this->config->get('quickcheckout_countdown');
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/quickcheckout/checkout.tpl')) {
			...
			$this->template = 'cosyone/template/quickcheckout/checkout.tpl';
		}
