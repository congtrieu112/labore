...
class ControllerModuleCosyoneCategory extends Controller {
	protected function index($setting) {
		...
		if (isset($parts[1])) {
			$this->data['category_2_id'] = $parts[1];
		} else {
			$this->data['category_2_id'] = 0;
		}
		
		if (isset($parts[2])) {
			$this->data['category_3_id'] = $parts[2];
		} else {
			$this->data['category_3_id'] = 0;
		}
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/module/cosyone_category.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/module/cosyone_category.tpl';
		} else {
			$this->template = 'cosyone/template/module/cosyone_category.tpl';
		}
