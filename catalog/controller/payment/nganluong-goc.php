...
class ControllerPaymentNganLuong extends Controller {
	/*protected function index() {
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/nganluong.tpl')) {
			...
			$this->template = 'cosyone/template/payment/nganluong.tpl';
		}	
	...
	protected function index() {
		...
		$_SESSION['link_pay'] = 'https://www.nganluong.vn/button_payment.php?receiver='.$receiver.'&product_name=MaDH: '.$order_code.' Mua hàng tại website '.str_replace('www.','',$_SERVER['HTTP_HOST']).' ('.date('d/m/Y').')&price='.$price.'&return_url=http://'.$_SERVER['HTTP_HOST'].'&currency=vnd';
