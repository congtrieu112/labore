...
<?php if ($delivery && (!$delivery_delivery_time || $delivery_delivery_time == '1' || $delivery_delivery_time == '3')) { ?>
   	<br /><span class="contrast_font input_heading"><?php if ($delivery_required) { ?><span class="required">*</span> <?php } ?><?php echo $text_delivery; ?></strong><br />
  	<input type="text" name="delivery_date" value="<?php echo $delivery_date; ?>" />
  	<?php if ($delivery_delivery_time == '3') { ?>
   
		<select name="delivery_time">
			<?php foreach ($delivery_times as $quickcheckout_delivery_time) { ?>
