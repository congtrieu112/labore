...
class ModelLocalisationLanguage extends Model {
	public function addLanguage($data) {
		...
		if ($this->config->get('qa_installed')) {
            		$this->load->model('setting/setting');

            		$qa_settings = $this->model_setting_setting->getSetting('qa');

		            if (isset($qa_settings['qa_form_custom_field_name'][$this->config->get('config_language_id')])) {
		                	$qa_settings['qa_form_custom_field_name'][$language_id] = $qa_settings['qa_form_custom_field_name'][$this->config->get('config_language_id')];
		            } else {
		                	$qa_settings['qa_form_custom_field_name'][$language_id] = '';
		            }

		            if (isset($qa_settings['qa_notification_emails'][$this->config->get('config_language_id')])) {
		                	$qa_settings['qa_notification_emails'][$language_id] = $qa_settings['qa_notification_emails'][$this->config->get('config_language_id')];
		            } else {
		                	$qa_settings['qa_notification_emails'][$language_id] = $this->config->get('config_email');
		            }

		            $this->model_setting_setting->editSetting('qa', $qa_settings);
        		}
