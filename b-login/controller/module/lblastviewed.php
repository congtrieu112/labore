...
class ControllerModuleLbLastViewed extends Controller {
	...
	public function index() {   
		$this->load->language('module/lblastviewed');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {			
			$this->model_setting_setting->editSetting('lblastviewed', $this->request->post);		
        ...
        if (isset($this->error['layout'])) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('module/lblastviewed', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);
		
		$this->data['action'] = $this->url->link('module/lblastviewed', 'token=' . $this->session->data['token'], 'SSL');
		
		$this->data['cancel'] = $this->url->link('extension/module', 'token=' . $this->session->data['token'], 'SSL');

		$this->data['token'] = $this->session->data['token'];
	
        if (isset($this->request->post['lblastviewed_count'])) {
			$this->data['lblastviewed_count'] = $this->request->post['lblastviewed_count'];
		} else {
			$this->data['lblastviewed_count'] = $this->config->get('lblastviewed_count');
		}	
				    
        $this->data['lblastviewed_count'] = isset($this->data['lblastviewed_count']) && !is_null($this->data['lblastviewed_count']) ? $this->data['lblastviewed_count'] : 5;
		...
		if (isset($this->request->post['lblastviewed_module'])) {
			$this->data['modules'] = $this->request->post['lblastviewed_module'];
		} elseif ($this->config->get('lblastviewed_module')) { 
			$this->data['modules'] = $this->config->get('lblastviewed_module');
		}		
		...
		$this->template = 'module/lblastviewed.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	private function validate() {
		if (!$this->user->hasPermission('modify', 'module/lblastviewed')) {
		...
		if (isset($this->request->post['lblastviewed_module'])) {
			...
			foreach ($this->request->post['lblastviewed_module'] as $key => $value) {
